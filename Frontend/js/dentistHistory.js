...
function isAuthenticated() {
    return sessionStorage.getItem("loggedIn") === "true" && sessionStorage.getItem("isDentist") === "true";
}

function renderUsername() {
    usernameElement.textContent = sessionStorage.getItem("username");
    usernameElement.href = "file:///D:/Bachelor%20Software%20HU/Projects/Project-2%20V.2/Frontend/html/dentistProfile.html";
}
...
async function fetchAppointmentHistory() {
    try {
        ...
        const response = await fetch(`http://localhost:3000/dentist/view-history?id=${id}`, {
            method: "GET",
            headers: {
        ...
        <div class="card">
            <div class="card-body">
                ...
                <div class="d-flex justify-content-end">
                    <button class="btn btn-primary mr-2" onclick="viewFeedback(${appointment.appointment_id})">View Feedback</button>
                    <button class="btn btn-success" onclick="giveTreatmentPlan(${appointment.appointment_id})">
                        ${appointment.has_treatment_plan ? 'Update Treatment Plan' : 'Give Treatment Plan'}
                    </button>
                </div>
            </div>
        </div>
...
function viewFeedback(appointmentId) {
    ...
    sessionStorage.setItem("viewFeedbackAppointmentId", appointmentId);
    window.location.href = "D:/Bachelor Software HU/Projects/Project-2 V.2/Frontend/html/viewFeedBack.html";
}

// Function to give treatment plan for a specific appointment
function giveTreatmentPlan(appointmentId) {
    // Redirect to the treatment plan page for the selected appointment
    sessionStorage.setItem("giveTreatmentPlanAppointmentId", appointmentId);
    window.location.href = "D:/Bachelor Software HU/Projects/Project-2 V.2/Frontend/html/giveTreatmentPlan.html";
}
